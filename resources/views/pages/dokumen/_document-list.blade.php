                ...
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2 flex-wrap">
                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-sky-50 text-sky-700 text-[11px] font-bold uppercase tracking-wider">{{ $doc['no'] }}</span>
                            @if(isset($doc['type']))
                            ...
                            @endif
                            @if(isset($doc['status']))
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold {{ $doc['status'] === 'Berlaku' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600' }}">
                                <span class="w-1.5 h-1.5 rounded-full mr-1 {{ $doc['status'] === 'Berlaku' ? 'bg-emerald-500' : 'bg-amber-500' }}"></span>
                                {{ $doc['status'] }}
                            </span>
                            @endif
                        </div>
...
<div class="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4" data-aos="fade-up">
    <p class="text-sm text-slate-500">
        Menampilkan <span class="font-semibold text-slate-700">{{ $offset + 1 }}-{{ min($offset + $perPage, $totalItems) }}</span> dari <span class="font-semibold text-slate-700">{{ $totalItems }}</span> dokumen
    </p>
...
<div class="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4" data-aos="fade-up">
    <p class="text-sm text-slate-500">
        Menampilkan <span class="font-semibold text-slate-700">1-{{ $totalItems }}</span> dari <span class="font-semibold text-slate-700">{{ $totalItems }}</span> dokumen
    </p>
</div>
@endif
@endif
