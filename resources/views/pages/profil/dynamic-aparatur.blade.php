<x-layouts.app>
    @php
        $subtitle = $profilePage?->content['subtitle'] ?? 'Data aparatur Bappelitbangda Kabupaten Mahakam Ulu.';
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
            <div class="bg-white rounded-2xl border border-slate-100 shadow-lg p-6 text-center">
                <div class="text-4xl font-black text-slate-900">{{ $aparaturs->count() }}</div>
                <div class="text-sm text-slate-500 mt-2">Total Aparatur Aktif</div>
            </div>
            <div class="bg-white rounded-2xl border border-slate-100 shadow-lg p-6 text-center">
                <div class="text-4xl font-black text-slate-900">{{ $aparaturs->pluck('position')->filter()->unique()->count() }}</div>
                <div class="text-sm text-slate-500 mt-2">Variasi Jabatan</div>
            </div>
            <div class="bg-white rounded-2xl border border-slate-100 shadow-lg p-6 text-center">
                <div class="text-4xl font-black text-slate-900">{{ $aparaturs->pluck('grade')->filter()->unique()->count() }}</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($aparaturs as $aparatur)
                <article class="bg-white rounded-3xl border border-slate-100 shadow-lg overflow-hidden hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div class="p-6">
                        <h2 class="text-lg font-bold text-slate-900">{{ $aparatur->name }}</h2>
