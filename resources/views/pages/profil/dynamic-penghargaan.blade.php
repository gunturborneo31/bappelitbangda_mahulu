<x-layouts.app>
    @php
        $subtitle = $profilePage?->meta_desc ?? 'Rekam prestasi dan penghargaan Bappelitbangda Kabupaten Mahakam Ulu.';
    @endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($awards as $award)
                <article class="bg-white rounded-3xl border border-slate-100 shadow-lg p-6 hover:-translate-y-1 transition-all duration-300">
                    ...
                    <h2 class="text-xl font-bold text-slate-900 leading-snug">{{ $award->title }}</h2>
