<x-layouts.app>
    @php
        $subtitle = $profilePage?->content['subtitle'] ?? 'Mengenal pimpinan Bappelitbangda Kabupaten Mahakam Ulu secara lebih dekat.';
    @endphp
    ...
    <section class="relative pt-[100px] pb-20 overflow-hidden bg-slate-900">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-20 text-center">
            <h1 class="font-montserrat font-black text-4xl sm:text-5xl lg:text-6xl text-white">Profil Pimpinan</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-10">
        @forelse($leaders as $leader)
            <section class="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
                <div class="grid grid-cols-1 lg:grid-cols-5">
                    <div class="lg:col-span-2 bg-slate-100 min-h-[380px]">
                        <img src="{{ $leader->photo ? asset('storage/' . ltrim($leader->photo, '/')) : asset('images/desamahakamulu.jpg') }}" alt="{{ $leader->name }}" class="w-full h-full object-cover">
                    </div>
                    <div class="lg:col-span-3 p-8 lg:p-10">
                        ...
                        <h2 class="text-3xl font-black text-slate-900">{{ $leader->name }}</h2>
            ...
            @if($leader->histories->isNotEmpty())
                <section class="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 lg:p-10">
                    <div class="flex items-center gap-3 mb-8">
                        ...
                        <h3 class="text-2xl font-bold text-slate-900">Riwayat Jabatan</h3>
                    </div>
                    <div class="space-y-6">
                        @foreach($leader->histories as $history)
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 border-b border-slate-100 pb-6 last:border-b-0">
            ...
            @endif
        @empty
            <div class="bg-white rounded-3xl border border-slate-100 shadow-xl p-10 text-center text-slate-500">Data pimpinan belum tersedia.</div>
        @endforelse
    </div>
</x-layouts.app>